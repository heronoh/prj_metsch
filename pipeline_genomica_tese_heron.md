...
``` bash
scaffold_stats.pl -f $genome.fasta -t 500 1000 -o $genome_stats.txt;
```
...
Remenber to check if the _genome.ann_ and _genome.dna_ files, generated by the *maker2zff* script are OK, cause they might be empty.
``` bash
...
#### Renaming maker outputs to more human friendly names

Maker CDSs have long bulky names so annotated CDSs must be renamed on *fasta* and *gff*.
I recommend something like the $genome_radical

The _--justify 4_ argument is to organize the gene numbers. If your genome has more than 9999 genes, use _--justify 5_.

Create the map first, a file with two columns, the old and the new name.
```bash
cd $genome.maker.output/
maker_map_ids --prefix $genome_radical --justify 4 $genome_radical.all.gff > $genome_radical_contig.map
```
Rename CDSs, proteins and GFF using the created map.
```bash
map_gff_ids $genome_radical_contig.map $genome_radical.all.gff; 
map_fasta_ids $genome_radical_contig.map $genome_radical.maker.proteins.fasta; 
map_fasta_ids $genome_radical_contig.map $genome_radical.maker.transcripts.fasta;" 
```
Check the files
